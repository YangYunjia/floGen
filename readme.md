...
This section introduce how to setup and run the prediction model.

## Dataset establishment

The first thing to do before training is to establish a dataset. The dataset is slightly different to the ordinary ones that is is a **seires dataset**.
...
Meanwhile, in order to train the model supervisedly, for each prior flowfield $f$, we also need many target flowfields. 
...
$$\mathcal S=\{r_f, \{x_{c,f}\}_{c=1,2,\cdots N_c(f)}\}_{f=1,2,\cdots,N_f}$$

where the flowfields for the same $f$ is called one **series**, or one group.

> Notice that the last subscript $N_c$ is a function of $f$, which means the number can be different for different airfoil. 

### Prepare data

The data should be storaged in to two `.npy` files in advance. (there are some examples in the folder `/app`).

#### data

The first data file contains the flowfields, should be named `data<yourname>.npy`, where `<yourname>` can be replaced to any legal string. It's shape should be 
...
- The second dimension is the number of channels. In flowfield reconstructing tasks, each channel can represent a mesh coordinate (i.e., x, y) or flow varialbe (i.e., p, T, u, v).

- The third dimension and rest dimensions are for each flowfield. If the input data is a profile (like a pressure profile on the airfoil surface), then it should be only the third dimension. If the input data is a field, then there should be the third and the forth dimension.

#### index

The first data file contains the index information of the flowfields, should be named `index<yourname>.npy`, where `<yourname>` is the same with in `data<yourname>.npy`. It's shape should be 

$$ (N_\text{prior flowfields} + N_\text{target flowfields}) \times N_\text{info} $$

For each flowfield, $N_\text{info}$ values can be stored. The should obey the format below:
...
|3 ~ 3+DC| $c$ | The condition values of the current flowfield (The length of this part depend on the dimension of a condition code)|
|4+DC~4+2DC| | The condition values of the prior flowfield of its series (The length of this part depend on the dimension of a condition code)|
|more| |Auxilary data|

### Import dataset with `ConditionalDataset`

The dataset should be construct before training with the following code:
...
>The `ConditionalDataset` is designed to do so: when `c_mtd` is other than `'load'`, a index list will be saved in the `data_base` folder with the name `<yourname>_<c_no>dataindex.txt`. So next time when you want to use this index map, simply use `c_mtd='load'` and assign the desired `c_no`.
...
|`c_mtd`|`str`| for each series, sometimes we want to choose *some* of it for training. This argument decide how to choose the conditions used in training<br/>- `fix` :   by the index in `c_map`<br/>-`random`: ramdomly selecte `n_c` conditions when initialize the dataset<br/>`all`:    all the conditions will be used to training<br/>`exrf`:   except the index of prior flowfield conditions<br/>`load`:   load the selection method by the file number `c_no` (use `save_data_idx(c_no)` to save)|
...
During training, the `ConditionalDataset` act like the origin `Dataset` in Pytorch. There are several extra functions you may interested in for post-process or other circumstance:
...
The backbone of the flowVAE is the Encoder - Decoder model or its variational version, VAE. To predict a series of off-design flowfields, the operating condition for the target flowfield need to be introduce. The model can be divided into three procedures as shown in the figure: 

![](.figure/flowvae2.png)

- The **Encoder** extracts *latent variables* (also can be seen as the statistic features) from the input prior flowfield.

- The **Concatenetor** combines the *latent variables* (only related to the prior flowfield) and the *operating condition* of the target flowfield. There are several stategy to do the above combination, both determistic and stochastic, and will be introduced in the sections below.

- The **Decoder** takes the concatenated vector as input, and generate the target flowfield.
...
For more details of the encoder and decoder, please see [here](#encoder--decoder)

### Set the model

Then we can construct the predicting model with the assigned encoder and decoder. This is done by construct a `frameVAE` or `Unet` class. They are arguments of them are same, like:
...
|`code_dim`| `int` | **Default**  `1`<br/> the condition code dimension (same with that in the dataset)
...
|`code_layer`|`List`|**Default**  `[]`<br/> (`code_mode` = `'semi'`, `'ex'`, `'ae'`, `'ed'`, `ved`, `ved1`)<br/> The layers between the real condition code input and where it is concatenate to the latent variables, see [Coder input layers](#code-input-layers)
...
|**Save & Load**
|`output_folder`|`str`|**Default**  `'save'`<br/> The path to setup a new folder called `opt_name` and save the checkpoints
|**Training**
|`init_lr`|`float`|**Default**  `'0.01'`<br/> The initial learning rate
|`num_epoch`|`int`|**Default**  `'300'`<br/> The maximum number of training epoch
|**Dataset**
|`split_train_ratio`|`float`|**Default**  `0.9`<br/> The ratio of the train dataset, the rest will be the validation dataset. The latter is not involve in the training, but is monitored to avoid over-fitting. Notice that the test dataset is split when constructing the `ConditionDataset`.
|`recover_split`|`str`|**Default**  `None`<br/> Whether to recover the split of the dataset (train and validation dataset). If is `None`, the dataset will be randomly split with the `split_train_ratio`, and the split result will be saved in `'<output_folder>/<optname>/dataset_indice'`. If not `None`, the split will be recover from `'<output_folder>/<recover_split>/dataset_indice'`<br/> 
|`batch_size` | `int` | **Default**  `8`<br/>  The mini-batch size (argument for `torch.utils.data.DataLoader`)|
|`shffle` | `bool` | **Default**  `True`<br/>  Whether to shuffle the dataset when training (argument for `torch.utils.data.DataLoader`)|
|**Model**
|`ref`|`bool`|**Default**  `False`<br/> Whether to use the *residual learning* strategy, i.e., to use residual value of the operating condition, and to reconstruct the residual field
...
|`recon_cahnnels`|`Tuple[int,int]`| **Default**  `(1, None)`<br/> The lower and upper range of the channels that to be compared with the output of the model. If the range is `None`, means on this direction there is no interception.<br/> *Remark.* The channel number should match the `out_channels` of the decoder
|`recon_type`| `str` | **Default**  `'field'`<br/> 

### Set loss parameters

Then, we also need to set some parameters for the loss calculation. There are several loss terms involved in training process. They are [basic loss terms]((#reconstruction-index-and-code-loss)) (include the reconstruction, code, and index loss) and the [physics-based loss terms](#physics-based-loss) (include the NS loss and the aerodynamic loss). 

When using different code concatent mode, the involved basic loss terms are not the same, and are concluded in the following table.
...
The physics based loss terms can be added as you want. It only related to the reconstructed flowfield.

To manipulate the loss parametets, you can use the following sentence:
...
|`indx_pivot`| `int` | ` -1`| the method to get avg_latent value <br/> - if is a positive number, use the latent value of that condition index <br/> - if `-1`, use the averaged value of all condition index

The arguments related to the physics based loss are:

|key | type | default | description|
|-|-|-|-|
|`sm_mode`|`str`|`'NS'`|the mode to calculate smooth loss<br/>   - `NS`:     use navier-stokes equation's conservation of mass and moment to calculate smooth<br/>   - `NS_r`    use the residual between mass/moment flux of reconstructed and ground truth as loss<br/>   - `offset`  offset diag. the field for several distance and sum the difference between field before and after move\n<br/>   - `1d`      one-dimensional data (adapted from Runze)
...
Then we need to set the optimizer and the scheduler to train the model. They are based on `torch.optim`, and assigned to the operator as follow:
...
Both the `set_optimizer` and `set_scheduler` have the first argument to be the *Class name* in `torch.optim` and `torch.optim.lr_scheduler`. Then the rest arguments should be the keywork arguments of the assigned optimizer and scheduler. Here is an example:
...
The optimizer is defaultly set to `'Adam'` with the `init_lr` assigned when initialize the `AEOperator`. If you want to change the optimizer, set it explicitly after construct the `AEOperator`.

The scheduler is recommanded to use the warmup strategy. It first increase the learning rate from a smaller value, and then reduce it. The flowVAE provide two warming up strategy in `flowvae.utils`
...
The flowVAE provide several useful functions to post process the reconstruct flowfield data. Most of them aim to obtain the aerodynamic coefficients of the surfaces in the flowfield. 
...
|-|-|-|-|
|`get_aoa(vel)`|extract the angle of attack(AoA) from the far-field velocity field| - `vel`:   the velocity field, shape: (2 x H x W), the two channels should be U and V (x and y direction velocity) <br/> Only the field at the front and farfield is used to averaged.|(`torch.Tensor`): the angle of attack
...
There are some functions to obtain the x,y direction force from a 1D pressure profiles. They are listed in the table below:

|name|description|arguments|returns|
|-|-|-|-|
|`get_xyforce_1d(geom: Tensor, profile: Tensor)` | integrate the force on x and y direction | - `geom`:    The geometry (x, y), shape: (2, N)  <br/> - `profile`: The pressure profile, shape: (N, ). It should be non_dimensional pressure profile by freestream condtion:  <br/> $$Cp = \frac{p - p_\text{inf}}{0.5\rho u^2} $$ | `Tensor` (Fx, Fy)|
...
There are several options of the Encoder & Decoder according to the flowfield being deal with. They are implentmented in `base_model.py` and with the framework of `torch.nn.Module`.

When using, the encoder and decoder should be defined explicitly before defining a `AEOperator`. For example:
...
> **Remark:** The flowfield has spacial correlations. The dense connected network ignores such correlations because it takes the adjust points in flowfields / profiles separately. Therefore, it is recommanded to use convolutional networks to capture the correlations and reduce the size of trainable parameters.
...
The dimensions of each hidden layers are defined with arguments `hidden_dims`. The length of the `hidden_dims` is the amount of the hidden dimension.

- `in_channels`: (`int`)
The input dimension of the first hidden layer. No matter the dimension of the input prior flowfield (1D or 2D), it will be flatten to one dimension before input to the encoder, so the `in_channels` for the `mlpEncoder` should be the flatten one. 

#### `mlpDecoder`

The encoder and decoder is defined by:
...
One dimensional convolution (1D conv) and two dimensional convolution neural network is promising solutions when dealing with 1D profiles (i.e., the pressure profiles on the airfoil) and 2D flowfields. They work mostly the same, and can be found in every deep neural network textbooks. The input to the 1D / 2D CNN can also have several channels, where each channel represents a flow varialbe (i.e., p, T, u, v).

#### `convEncoder`

The encoder consists of several encoder blocks. Each of the block contains a 1D / 2D convolutional layer (`nn.Conv1d` or `nn.Conv2d`), an activation layer (For default, we use the `nn.LeakyReLU`. It can be changed by assigning `basic_layers['actv'] = nn.ReLU`). 
...
- kernel size (`kernel_sizes[i]`), stride (`strides[i]`), and padding (`paddings[i]`). For 1D, they are `int`; for 2D, `int` means they are same for height and width direction, `tuple` means they are different.
- if there is a average pool layer, additional pool kernel size (`pool_kernels[i]`) and pool stride (`pool_strides[i]`) are needed.
...
- $ B_i $ is size the mini-batch
- $ C_i $ is the channel size, given by the argument `hidden_dim[i]`
- $ H_i (\times W_i) $ is the size of the feature map. It can be 1D or 2D. They are given by the parameters of the convoluation kernal as follow:
...
Also if necessary, there can be batch normalization layers in every blocks. This can be done by assigning `basic_layers['bn'] = nn.BatchNorm1d` or `basic_layers['pool'] = nn.BatchNorm2d`. Here we code to put the batch normalization layers between the convolutional layers and the activation layers. Some researches shows that locating the BN layers after the activation layers may lead to better performances, but we didn't find to much benifits.
...
- `in_channels`: (`int`)
    ...
    The size of channels of the profiles input to the encoder. Each channel can represent a flow varialbe (i.e., x, y, p, T, u, v)

- `last_size`: (`List` of `int`)
    
    The size of the feature map that closest to the latent variables (for encoder is the last feature map). Notice that the list does not contain the channel size.

- `hidden_dims`: (`List` of `int`)
    
    The channel size of the feature maps. The length of the `hidden_dims` determines the amount of encoder blocks.
...
The decoder also consist of several blocks. Each of the block realize upscaling with three layers. First, a linear (for 1D) or bilinear (for 2D) interpolation layer (`F.interpolate`) expands the feature map to a given size `sizes[i]`. 

Then, a 1D / 2D convolutional layer is applied to modify the interpolation results. The size of feature map is maintained the same for the convolutional layer, so the stride of the convolution kernel is 1, and the padding is given by $ p = (k - 1) / 2$, where $k$ should be a even number.

The last is the activation layer. Like the encoder, the default is a `nn.LeakyReLU` layer, and can be changed by assigning `basic_layers['actv']`. The batch normalization layer is also avilable if needed, by assigning `basic_layers['bn']`.

The upscaling process can also be done with `nn.ConvTranspose2d` for example, but they are not implenmented in the flowVAE.

![](.figure/1dconvdecoder.png "Sketach of the 1d convolution network decoder")

For each block, the channel size ( $C$ ) is given by `hidden_dims[i]`. The feature map ( $H (\times W)$ ) size is explicitly given by argument `sizes[i]`. For 1D cases, the `size[i]` should be a `int`, and for 2D cases, the `size[i]` should be a `List` contains two `int` (for $H$ and $W$).
...
- `out_channels`: (`int`)
    ...
    The size of channels of the decoder's output. It not always the same as `in_channels`, because that usually we input the geometry channels as well as the flow varialbes to the encoder, but only want the decoder to reconstruct the flow varialbes.

- `last_size`: (`List` of `int`)
    
    The size of the feature map that closest to the latent variables (for decoder is the size input to the first decoder block). Notice that the list does not contain the channel size.

- `hidden_dims`: (`List` of `int`)
    
    The channel size of the feature maps. The length of the `hidden_dims` determines the amount of decoder blocks.

- `sizes`: (`List` of `int`)
    
    The sizes of each feature maps. (see above)

- `basic_layers` [**Default:** see below]: (`Dict`)
    
    Basic layers to construct the decoder. The table below shows the fundamental layers and their default values. If one want to change the default one, the value for the corresponding key should be input to the encoder with the `Class` object.
...
The U-Net improve model prediction performances in the way like the ResNet: introducing direct identical links (skip connection) to avoid gradient diminishing. Meanwhile, it won't increase the parameter size of the model, and will only raise a little training time cost. It is recommanded to use U-Net version 1D-conv network by default.

The U-Net in flowVAE is implemented with separate classes `convEncoder_Unet` and `convDecoder_Unet`. They inherite the vanilla `convEncoder` and `convDecoder`. 
...
The arguments of the U-Net version is mostly the same with the vanilla one, while there is one extra argument for the `convDecoder_Unet`:
...
ResNet is a popular CV backbone network [Ref. He (2016)]. It is simple, but its performance is outstanding. It improve model predicting performances by adding skip connections (shortcut) between layers to avoid gradient diminishing. Here, the ResNet is invoked to construct the encoder and decoder.

#### Residual block

The minimum unit of the ResNet is **Residual Block (RB)** that implenment the skip connection. Its structure is depicted in the following figure.

![](.figure/rb.png "Residual block")

There are two connection paths between the input and output feature maps:

The lower is the **main path**, consist of two groups of the 2D convolutional layers. The kernel parameters of the convolutional layers are fixed to kernel size ( $k$ ) = 3, stride ( $s$ ) = 1, and padding( $p$ ) = 1. Therefore, the size of the output feature map is the same with the input. The upper is a **shortcut path** which directly connect the input and output feature maps, ensuring the propagation of the value and gradients.

There are also batch normalization layer and activation layers after each convolutional layer. Due to the BN layer, the `bias` is shut down in the convolutional layer. 
...
> The flowVAE support the pre-activation version by set the argument `preactive = True`. Yet, it didn't show much improvement to the test cases.
...
There are still two paths in the RBDS and RBUS. In order to change the size of the output feature maps, the kernel parameters for the first convolutional layer is modified. 

For the RBDS, it is modified to a kernel with kernel size ( $k$ ) = 3, <u>stride ( $s$ ) = 2 </u>, and padding( $p$ ) = 1. Meanwhile, the number of channels is increase to the given `hidden_dims[i]` to maintain the conservation of information flux.

For RBUS, the first convolutional layer is replaced with a up-sample layer that is similar to what we used in the `convDecoder`. It contains a bilinear (for 2D) interpolation layer (`F.interpolate`) with `scale_factor = 2` to double the height and width of the feature map; and a 2D convolutional layer with kernel size = 3, stride = 1, and padding = 1. The convolution layer also reduces the number of channels to the given `hidden_dims[i]`.

> **Interpolation layer:**  The interpolate layer is implemented with the class `IntpConv`. It has one-dimensional version `IntpConv1d` and two-dimensional version `IntpConv2d`. They all use `F.interpolate` to up-sampling and down-sampling the feature map into the given sizes. The `F.interpolate` has two mode: assign the `size` or assign the `scale_factor`. When the `scale_factor` is assignned, the output size will be calculated with 
...
The encoder is constructed with several groups, where each group consist of a RBDS and several RBs. 

![](.figure/resnetencoder.png)

In each group, the input feature maps first go through the RBDS. Inside the RBDS, the feature map's size is reduced with the given `strides[i]` (normally it is 3, so the size will be halved). Meanwhile, the channel number is increased to the given `hidden_dims[i]`. Then the feature maps go through several RBs. The amount of RBs is `num_blocks[i] - 1`. Inside the RBs, the feature map sizes and channel number remains the same. The amount of group is determined by the length of the `hidden_dims`. 

Before the residual block groups, an optional convolutional layer (called the `extra_first_conv`) can be added to manipulate the input channel number to a desired one. It is done by assigning the argument `extra_first_conv` to a `Tuple` with four element: (output hidden dimension, kernel size, stride, padding).

After the residual block groups, the feature map size should be the same as assigned in `last_size`. If the `force_last_size` is set to `True`, an additional adaptive average pooling layer is applied to modify the size to given one. Else, there will be a error raised if the `last_size` mismatch the real output size.
...
- `hidden_dims`: (`List` of `int`)
    ...
    The channel size of the feature maps for each residual block groups (after every RBDS). The length of the `hidden_dims` determines the amount of encoder groups.

- `num_blocks`[**Default:** `2` for each group]: (`List` of `int`)
    
    The number of residual blocks (include the RBDS and RB) of each groups. Then there will be one RBDS and (`num_block` - 1) RBs in that group. Every element in the list should >= 2.

- `strides` [**Default:** `2` for each group]: (`List` of `int`)
    
    The strides of each convolutional layer in the RBDS block in every groups.
...
- `force_last_size` [**Default:** `False`]: (`bool`)
    ...
    Whether to add an additional layers after the residual block groups in order to force the output feature map into the given `last_size`.
...
The ResNet decoder is similar to the encoder.The input latent vector is first reshape to feature maps with assigned `last_size`. Then they will go through several groups, where each group consist of a RBUS and several RBs. 

![](.figure/resnetdecoder.png)

In each group, the input feature maps first go through the RBUS. Inside the RBUS, the feature map's size is increased with the given `scale_factor[i]` (should greater than 1, and normally it is 2, so the size will be doubled). Meanwhile, the channel number is decreased to the given `hidden_dims[i+1]`. Then the feature maps go through several RBs. The amount of RBs is `num_blocks[i] - 1`. Inside the RBs, the feature map sizes and channel number remains the same. The amount of group is determined by the length of the `hidden_dims`. 

After the residual block groups, the feature map size can be manipulated to a given output size if the argument `output_size` is given. If so, an additional interpolation layer (same as the up-sampling layer in the RBDS) is applied to modify the size to given one. 
...
- `hidden_dims`: (`List` of `int`)
    ...
    The channel size of the feature maps for each residual block groups (after every RBUS). The length of the `hidden_dims` determines the amount of decoder groups.

- `num_blocks`[**Default:** `2` for each group]: (`List` of `int`)
    
    The number of residual blocks (include the RBUS and RB) of each groups. Then there will be one RBUS and (`num_block` - 1) RBs in that group. Every element in the list should >= 2.

- `scales` [**Default:** `2` for each group]: (`List` of `float`)
    
    The scale factor of each interpolation layer in the RBUS block in every groups.

- `output_size` [**Default:** `None`]: (`List` of `int`)

    If this argument is assigned for a value other than `None`, an additional layers will be added after the residual block groups in order to force the output feature map into the given `output_size`.
...
The concatenetor's target is to combine the **latent variables** extracted from the encoder with the **operating conditions**. There are several solutions, and there are selected through the arguments when construct the object of the model class (`frameVAE` or `Unet`)

### Concatenetion strategy

Generally speaking, there are two kinds of concatenetion strategy. The difference of these two strategy is majorly in the training process. Let's consider that we are training a model to predict the flowfield of an arbitary airfoil under a arbitary angle of attack (AoA). The prior input is this airfoil's flowfield under the cruise angle of attack (AoAc). 

Before training, we should have a "series" database (the database will be introduced in the Pre-process section). This means, we have a database of many different airfoils ( $foil_1, foil_2, \cdots, foil_{N_f}$ ), and for each airfoil $f$, we have many flowfield of this airfoil under many different operating conditions (AoA) $c_1, c_2, \cdots, c_{N_c(f)}$. Notice that the last subscript $N_c$ is a function of $f$, which means the number can be different for different airfoil. Then, each flowfield in the database can be written as 
...
The model need to construct the following mapping from training:

$$ \mathcal F: (r_f, c) \mapsto x_{c,f}~~\forall c, f$$

Split the mapping above into the encoder $\mathcal E$ and the decoder $\mathcal D$:

$$ \mathcal E: r_f \mapsto z_{f}~~\forall f \\
\mathcal D: (z_f, c) \mapsto x_{c,f}~~\forall c,z_f $$

Then, during training, we will have two strategies to utilize the flowfield in the database:

#### Auto-encoder perspective

From principles of the auto-encoder, the model's target is to **reconstruct** the input flowfield. From this perspective, the model can be seen as a dimension reduction tool, to extract low-dimensional representation (latent variables) $z$ from the flowfield.
...
In order to reach this, we split the latent variables to two part: the code part $z_c$ for operating condition and the flow feature part $z_f$. The following figure gives a rough description of the model. The encoder extract flow feature part from both the design and off-design flowfields. Then they are combined with the code part to form the complete latent variable $z$ and input to the decoder.

![](.figure/aeframe.png)

Now, we only need to ensure that the flow feature part latent variable (generated by the encoder) fulfills $ \mathcal E(x_{c,f}) \rightarrow \mathcal E(r_f), ~~ \forall c,f$. In other word, the $z_f$ for the off-design variable should be the same as the design variable (the prior latent variables). For simplify, we note it as $z_f \rightarrow z_{r}$. This is realized by adding additional loss terms (code loss) into the training process, as the `MSE loss` in the figure.

However, during training, the flowfields are input to the model in random mini-batch, so that we can't ensure the flowfields for one airfoil are in the same mini-batch. Thus, we can't ensure to obtain the up-to-date $z_r=\mathcal E(r_f)$ during training. To solve this problem, we proposed an **iterative training algorithm**: the prior latent variables $z_r$ are obtained after one epoch with the current encoder. The algorithm is illustrated below:
...
>>> 6. get loss $l = \mathcal L_1(x_{c,f}, x'_{c,f}) + \mathcal L_2(z_{f}, z_{r})+\cdots$
>>> 7. back-propogation and update model trainable parameters
>>> 8. return to 4. if there remains flowfields
>> 9. update prior latent variables: $z_r = \mathcal E(r_f)$
>> 10. return to 3. if not convergent

In the above figure, the latent variables are determinstic. The can also be stochastic, which means the $z_f$ is be seen as a normal distribution $z_f \sim \mathcal N(\mu_f, \sigma_f)$, and the encoder is to predict the distribution parameters $\mu_f, \sigma_f$ instand of predicting the $z_f$. For decoder, the reparameterization trick is applied to sample from the distribution and give a reconstruct flowfield. 

Also, the way to deal with the operating conditions can also be different. Here we just put the real operating condition into the decoder and it is not put into the encoder part. Sometimes we want the encoder also takes the OC information when dealing with off-design flowfields. This lead to another several types of the concatnetor as shown below:

- Implicit: the encoder predict the $z_c$, and another code loss is added to implicitly force the code given by encoder to match the real code.
...
- Explicit: the encoder don't predict the $z_c$, and the real code is input to the last encoder layer, while it is also input to the decoder.
- None: the real code has no relation with the encoder.

![](.figure/autoencoder.png)

The advantage of auto-encoder perspective is it has clear theory basis (especially when using the Bayesian theory). The theoratical analysis can be found in the reference papers. The disadvantages is that it performs far worth than the encoder-decoder perspective below.

#### Encoder-decoder perspective

The difference of the encoder-decoder perspective is that it takes the encoder and decoder separately, and it do not require the identity of the output and input flowfield. So the target of the encoder-decoder model is to **generate** the off-design flowfield with whatever input. The input can be the mesh only (this is what most flowfield generators do), and can be the prior flowfield.

The following figure gives a rough description of the model. The encoder extract flow feature $z_f$ from only off-design flowfields. Then they are combined with the code part $z_c$ to form the complete latent variable $z$ and input to the decoder.

![](.figure/edframe.png)

We shall notice that the the $z_f$ is naturally the same for the same airfoil, so this time we don't need to add the additional loss term to ensure the flow feature part of the latent varialbes. The iterative training algorithm is also of no use. 

There are also varianal version of the model, which is depicted in the following figure:

![](.figure/encoderdecoder.png)

### Extra layers

There are some extra layers in the concatentor to bridge diffent hidden dimension and to improve the predicting performances. These extra layers can be changed according to needs.

#### Decoder input layers

Extra layers can be added between the concatented latent variables $z$ and the input of the decoder. This is initially to bridge the different dimension of the latent variables (`ld` = $D_{z_c} + D_{z_f}$) and the dimension needed for the decoder (`lfd` = `decoder.last_flatten_size`). This can be done with one dense connected layer. Increasing the number of the decoder input layers may have some benifit, but experiments shows that there won't be much benefits.
...
|-|-|-|
|`0`|`nn.Identity()`|Default for dense connected decoder `mlpDecoder`, because the input dimension of the `mlpDecoder` is usually assigned as the dimension of the latent variables ($D_{z_c} + D_{z_f}$), so there is no need to bridge the difference.
|`1`| `nn.Linear(ld, lfd)`| Default for other decoder. Use a single linear layer to bridge the differce in different dimension.|
...
Extra layers can also added between the input condition code ( $c$, dimension = $D_c$ ) and the latent variable for the code ($z_c$, dimension = $D_{z_c}$). The purpose is to simulate the correlation among different elements of the condition code.

To construct the code input layers, you may assign the argument `code_layer` with a `List` containing a squential of `int` numbers. Then a multi-layer neural network will be constructed, and each `int` number represents a dense connected layer with such hidden dimension. 
...
The basic loss terms include the reconstruction, code, and index loss. When using different code concatent mode, the involved loss terms are not the same, and are concluded in the following table.
...
The reconstruction loss is universal for all mode and is given by

$$ \mathcal L_\text{reconstruct} = \frac 12 ||x-\hat x||^2$$

The code loss is used for mode that need to implicitly forcing the code generate by the encoder to be the same as thereal code. It is given by 

$$ \mathcal L_\text{code} = \frac 12 ||\mu_c-c||^2$$

The index loss is a bit conplex. It is used to control the flow-feature part latent variables $z_f$ to satisfy restrictions. For different concatenent mode, the restrictions are different:
...
- **MSE-p**:  Mean square error to the prior latent variables $z_r$. This kind of index loss is applied to `ae`, in order to force the determinstic $z_f \rightarrow z_r$. Suppose they both obey the normal distribution, then we have:

    $$ \mathcal L_\text{index} = MSE\left[\mathcal z_f, z_r\right] =\frac 12 ||z_f-z_r||^2$$

- **KL-n**：  KL divergence to the standard normal distribution $\mathcal N(0,1)$. This can be seen to the original version of the varience operation. It is applied to `ved` and `ved1`, to give a stochastic version of the encoder-decoder's latent variables. The flow-feature latent variables that the encoder extracts from the prior flowfield is seen as a normal distribution, and the KL loss will force it to the standard normal distribution:

    $$ \mathcal L_\text{index} = KL\left[\mathcal N(\mu_f, \sigma_f), \mathcal N(0, 1)\right] =KL=-\frac{1}{2} \sum_{j=1}^{J}\left(1+\log \sigma_j^{2}-\sigma_{j}^{2}-\mu_{j}^{2}\right)$$
...
Conservation of mass, momentum, and energy are fundamental physical laws that apply to fluid flow. Since the calculation of momentum and energy losses involves information of the turbulence fields, which are not predicted by the model, only mass conservation loss is considered in this paper. 

The mass flux is calculated cell by cell for each reconstructed flowfield and is minimized by adding a corresponding term to the loss function. In the framework of the finite volume solver, the flow variables are stored at the cell center. The flow variables are interpolated from the cell center to the grid edge to calculate the mass flux, as shown in figure.
...
$$\mathcal{L}_{\text {Mass }}=\Phi_{\mathrm{m}}=\max \left(0, \sum_{i j} \Phi_{\mathrm{m}, i j}-\Phi_{\mathrm{m}, i j}^{(\mathrm{real})}\right)$$
