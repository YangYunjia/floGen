...
class Ploter():
    ...
    def __init__(self, name: str = None, symbol: str = 'o', color: str = 'r') -> None:
        self.name = name
        self.symbol = symbol
        self.color = color
...
class Buffet():
    ...
    def __init__(self, logtype=0, **kwargs) -> None:
        ...
        if 'lsuth2' not in kwargs.keys():       self.paras['lsuth2'] = 1.2      # linear segment upper bound therhold lower
    ...
    def _estimate_linear_lift_curve(self, AoAs: np.array, CLs: np.array, CDs: np.array = None, X1s: np.array = None, cl_c: float = 0.8, add_right: int = 0) -> dict:
        ...
        if self.paras['lsumtd'] in ['error']:
            ...
            while max_aoa > aoa_refs[max_i] + self.paras['lsuth1'] - self.paras['lsuth2'] and max_aoa > AoAs[i_lb + 2]:
                min_aoa = max(AoAs[i_lb - 1], max_aoa - self.paras['lslt'])
        ...
        self.log(' > Predict linear section slope: %.4f [%.4f, %.4f]; intercept: %.4f' % (low_slope, low_slope, high_slope, intercept))
    ...
    def _estimate_incipient_separation(self, AoAs, mUys, kind=1):
        ...
        if fUy is not None:
            ...
            if fUy(A_upp)>=0:
                ...
                aa = np.arange(A_low, A_upp, 0.001)
                uy = fUy(aa)
    ...
    def lift_curve_break(self, seri: Series, cl_c: float = 0.8, p: Ploter = None):
        '''
        ...
        for low_bound in [AoA_sep, ll['AoA_ub'] - 0.1]:
